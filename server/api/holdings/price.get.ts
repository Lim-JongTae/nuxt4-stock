...
export default defineEventHandler(async () => {
  ...
  const now = new Date();
  const localTime = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')} (LS증권 API)`;

  const result = await Promise.all(items.map(async item => {
    const latestPrice = token ? await getLSPrice(token, item.shcode) : null;
    const curPrice = latestPrice || item.currentPrice || item.avgPrice || 10000;

    // Simple dynamic target/stop calculations (same logic as holdings API)
    const dynamicTargetPrice = Math.round(curPrice * 1.072);
    const dynamicStopLossPrice = Math.round(curPrice * 0.948);
