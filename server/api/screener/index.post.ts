...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Parallel fetch live prices if token is available
    if (token) {
    ...
    console.error('Screener endpoint error:', err);
    const fallbackTime = new Date().toLocaleString('ko-KR');
    return {
      success: true,
      timestamp: fallbackTime,
      source: 'LS증권 API (백업 데이터 모드)',
      oldData: [],
      newData: []
    };
  }
